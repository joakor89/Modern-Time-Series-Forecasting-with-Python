...
def mape(actuals, predictions):
    return np.nanmean(np.abs((actuals - predictions) / np.where(actuals == 0, np.nan, actuals))) * 100
...
def smape(actuals, predictions):
    ...
    return np.nanmean(np.abs(actuals - predictions) / np.where(denominator == 0, np.nan, denominator)) * 100
